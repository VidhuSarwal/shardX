...
- [ ] **SQS + DLQ for shard upload retries — IN FLIGHT.** A background agent (task description: "Add SQS+DLQ retry worker to ShardX") was running when this doc was last updated. Check for a worktree at `.claude/worktrees/agent-*` before starting fresh — if one exists with uncommitted work, read its report/diff before deciding whether to finish merging it or restart. If no worktree exists and no clear result was reported, treat this as not-yet-done and implement `internal/queue/` (Queue interface + SQSQueue + NoopQueue), wire `processAndUploadFile`'s S3-mode chunk-upload failure path to enqueue a retry job instead of aborting the whole session, and build a standalone `cmd/shardworker/main.go` consumer. Follow the exact same `eventEmitter`/`InitEvents` package-level-var pattern already used in `internal/filehandlers/handlers.go` for `orchestrator`/`metaStore` — add an `activeStorageProvider`/`InitStorageProvider` following that pattern too. Drive-mode fail-fast behavior must stay byte-for-byte unchanged.
